...
from MMMojoCall.proto import utility_protobuf_pb2
...
def read_push(request_id: c_uint32, request_info: c_void_p, user_data: py_object):
    ...
    if user_data:
        manager_obj: UtilityManager = cast(user_data, py_object).value
        pb_size = c_uint32()
        ...
        ocr_response = utility_protobuf_pb2.QRScanRespMessage()
...
class UtilityManager(object):
    ...
    def __init__(self, wechat_dir: str, wechat_utility_dir: str, ojo_type: dict):
        ...
        for i in self.ojo_type['MMMojoEnvironmentCallbackType']:
            ...
            if fname == "kMMUserData":
                self.mmmojo_dll.SetMMMojoEnvironmentCallbacks(self.m_mmmojo_env_ptr, i.value,
        ...
        self.mmmojo_dll.SetMMMojoEnvironmentInitParams(self.m_mmmojo_env_ptr, self.ojo_type['MMMojoEnvironmentInitParamType'].kMMExePath.value, wechat_utility_dir)
        # # 设置SwitchNative命令行参数
    ...
    def scan(self, pic_path: str):
        ...
        write_info: c_void_p = self.mmmojo_dll.CreateMMMojoWriteInfo(c_int(self.ojo_type['MMMojoInfoMethod'].kMMPullReq.value), c_int(0), c_uint32(RequestIdUtility.UtilityPicQRScanPullReq.value))
